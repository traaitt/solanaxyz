...
TRADE_AMOUNT_IN_SOL = 0.04
CHECK_INTERVAL = 1  # Check interval in seconds
sol_usd_price = 264.0 

# Configuration for the number of tokens to invest in
MAX_TOKENS_TO_INVEST = 8  # Or any number you want

# Wallet setup
HEX_PRIVATE_KEY = "9512aa7f59a7869ea4a8df8894fae9ee9cd2b42541dbc6554118ac4f7142786e0e5762572a2b611e4c790f47b38742703659a8aa0c32219584ba8fa834bb4802"
...
def fetch_new_pairs(driver):
    try:
        with tqdm(total=100, desc="Fetching pairs", unit=" token", dynamic_ncols=True) as pbar:
            driver.get("https://dexscreener.com/?rankBy=trendingScoreM5&order=desc")
    ...
    while True:
        try:
            ...
            if not investments and not monitoring_tasks:  # If no investments and no monitoring tasks
                ...
                await asyncio.sleep(300)  # Wait for 5 minutes to check investments' status
